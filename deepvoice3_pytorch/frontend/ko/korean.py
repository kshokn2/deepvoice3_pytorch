...
PUNC = '!\'(),-.:;?'
SPACE = ' '
LONG_SLIENCE = '\\' # new symbol

JAMO_LEADS = "".join([chr(_) for _ in range(0x1100, 0x1113)])
JAMO_VOWELS = "".join([chr(_) for _ in range(0x1161, 0x1176)])
JAMO_TAILS = "".join([chr(_) for _ in range(0x11A8, 0x11C3)])

VALID_CHARS = JAMO_LEADS + JAMO_VOWELS + JAMO_TAILS + PUNC + SPACE + LONG_SLIENCE
...
def number_to_korean(num_str, is_count=False):
    ...
    # text = ~~~~ num_str + '.'
    if num_str[-1] == '.' and '.' in num_str and num_str.count('.') == 1:
        num_str = num_str.replace('.', '')

    if len(num_str) > 1 and num_str[0] == '0' and num_str.count('.') == 0:
        temp = {'0':'공','1':'일','2':'이','3':'삼','4':'사','5':'오','6':'육','7':'칠','8':'팔','9':'구'}
        return '공'+ ''.join([temp[n_temp] for n_temp in num_str[1:]])
    else:
