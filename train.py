...
def plot_alignment(alignment, path, info=None, show_korean=None):
    fig, ax = plt.subplots()
    im = ax.imshow(
        alignment,
        aspect='auto',
    ...
    if show_korean is not None:
        fontprop = matplotlib.font_manager.FontProperties(fname='/usr/share/fonts/NanumGothic.ttf')
        plt.yticks(range(len(show_korean)), show_korean, fontproperties=fontprop, fontsize=9)
    plt.tight_layout()
